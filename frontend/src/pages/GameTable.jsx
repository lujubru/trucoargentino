...
const GameTable = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a4d2e] to-[#0f2818] flex flex-col overflow-hidden">
      ...
      <header className="bg-[#0d1f14]/80 backdrop-blur border-b border-white/10 px-3 py-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-center">
              <p className="text-[8px] text-[#2ECC71] uppercase tracking-wide">Nos:</p>
            ...
            <div className="text-center">
              <p className="text-[8px] text-[#E74C3C] uppercase tracking-wide">Ella:</p>
              <div className="flex gap-0.5 mt-0.5">
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
      ...
      <div className="flex-1 relative overflow-hidden" style={{ perspective: '1000px' }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            {/* Table surface */}
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-[#8B4513] via-[#A0522D] to-[#654321] shadow-2xl border-4 border-[#654321]/50"
              style={{
                transform: 'translateZ(-20px)',
                boxShadow: '0 30px 60px rgba(0,0,0,0.5), inset 0 2px 20px rgba(255,255,255,0.1)'
              }}
            >
            ...
            {game?.players?.map((player, index) => {
              ...
              let positionStyle = {};
              if (position === 'top') {
                positionStyle = { top: '-60px', left: '50%', transform: 'translateX(-50%) rotateX(-45deg)' };
              } else if (position === 'bottom') {
                positionStyle = { bottom: '-60px', left: '50%', transform: 'translateX(-50%) rotateX(-45deg)' };
              } else if (position === 'left') {
                positionStyle = { top: '50%', left: '-60px', transform: 'translateY(-50%) rotateX(-45deg)' };
              } else if (position === 'right') {
                positionStyle = { top: '50%', right: '-60px', transform: 'translateY(-50%) rotateX(-45deg)' };
              } else if (position === 'top-left') {
                positionStyle = { top: '10%', left: '-50px', transform: 'rotateX(-45deg)' };
              } else if (position === 'top-right') {
                positionStyle = { top: '10%', right: '-50px', transform: 'rotateX(-45deg)' };
              } else if (position === 'bottom-left') {
                positionStyle = { bottom: '10%', left: '-50px', transform: 'rotateX(-45deg)' };
              } else if (position === 'bottom-right') {
                positionStyle = { bottom: '10%', right: '-50px', transform: 'rotateX(-45deg)' };
              }
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center gap-1">
                    <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center text-white font-bold text-lg border-4 ${
        ...
        <AnimatePresence>
          {waitingForResponse && (
            ...
            >
              <div className="bg-gradient-to-br from-[#1a4d2e] to-[#0f2818] p-6 rounded-2xl border-2 border-[#FFD700] max-w-sm w-full">
                <p className={`font-display text-3xl md:text-4xl mb-2 text-center ${
                  waitingForResponse.type === 'truco' ? 'text-[#E74C3C]' : 'text-[#FFD700]'
                }`}>
                  ¡{waitingForResponse.call.replace('_', ' ').toUpperCase()}!
                </p>
                <p className="text-white text-center mb-6">Vale {waitingForResponse.points} puntos</p>
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    onClick={() => waitingForResponse.type === 'truco' ? handleRespondTruco('quiero') : handleRespondEnvido('quiero')}
                    className="bg-[#2ECC71] hover:bg-[#27AE60] text-white font-bold py-6 text-lg"
                  >
                    QUIERO
                  </Button>
                  <Button
                    onClick={() => waitingForResponse.type === 'truco' ? handleRespondTruco('no_quiero') : handleRespondEnvido('no_quiero')}
                    className="bg-[#E74C3C] hover:bg-[#C0392B] text-white font-bold py-6 text-lg"
                  >
                    NO QUIERO
                  </Button>
                </div>
          ...
          <div className="flex gap-2">
            {myHand?.cards?.map((card, idx) => (
              <motion.div
                ...
                animate={{ 
                  opacity: 1, 
                  y: selectedCard === idx ? -20 : 0,
                  rotateY: 0,
                  scale: selectedCard === idx ? 1.15 : 1
                }}
      ...
      <div className="bg-[#0d1f14]/90 backdrop-blur border-t border-white/10 p-2">
        <div className="grid grid-cols-4 gap-2 max-w-2xl mx-auto">
          <Button
            onClick={() => canCallTruco ? handleCallTruco('truco') : canRaiseTruco && game?.truco_state === 'truco' ? handleCallTruco('retruco') : null}
            disabled={!canCallTruco && !canRaiseTruco}
            className="bg-[#E74C3C] hover:bg-[#C0392B] text-white font-display py-6 text-sm md:text-base disabled:opacity-40"
          ...
          <Button
            ...
            className="bg-[#FFD700] hover:bg-[#FFC700] text-[#0f2818] font-display py-6 text-sm md:text-base disabled:opacity-40"
          ...
          {canCallFlor ? (
            <Button
              ...
              className="bg-[#9B59B6] hover:bg-[#8E44AD] text-white font-display py-6 text-sm md:text-base"
          ...
          <Button
            ...
            className="bg-[#34495E] hover:bg-[#2C3E50] text-white font-display py-6 text-sm md:text-base"
          >
            MAZO
          </Button>
        </div>
        ...
        {isMyTurn && canPlayCard && (
          <p className="text-[#FFD700] text-center text-xs mt-2 animate-pulse font-bold">
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            className="fixed inset-x-0 bottom-0 bg-[#0d1f14] border-t border-white/10 z-40 h-[60vh] flex flex-col"
          >
